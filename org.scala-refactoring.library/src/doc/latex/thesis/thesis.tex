...
As explained at the beginning of Chapter~\vref{chapter:implemented-refactorings}, a refactoring is performed in several steps; \figref{figure:refactoring-sequence} presents the interaction between the user, the IDE and the refactoring library. The prepare and perform methods as well as the required parameters are part of the \src{MutliStageRefactoring} abstract class (also see \figref{figure:package-layout}).
...
The \src{EditorHelpers} object contains additional methods that make some of the often used Eclipse functions make suitable for Scala -- working with \src{Option} instead of \src{null} and helper functions similar to the \src{withCompilerResult} method above:
...
The \src{ExtractMethodScalaIdeRefactoring} class then adapts the library's Extract Method. All the non-refactoring specific code like calling \src{perform} is done in \src{ScalaIdeRefactoring}.
...
\begin{enumerate}
  \item Subclass \src{ScalaIdeRefactoring} and implement the necessary methods, analog to the existing IDE refactoring implementations.
  \item Create a \src{RefactoringAction} subclass that instantiates the previously created IDE refactoring.
  \item Add the new \src{RefactoringAction} to the plugin xml.
\end{enumerate}
...
This appendix provides an introduction for developers that are interested in developing new refactorings. We will create a new refactoring step-by-step but refer to the main documentation for details and background information.
