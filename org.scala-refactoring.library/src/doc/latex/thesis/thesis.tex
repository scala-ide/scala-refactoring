...
Refactoring means to improve the internal structure of a program while keeping its external behavior. Improving the internal structure can be done in various ways: the names that are used internally can be changed to better reflect their functionality, or the code can be reorganized to make it easier to extend, read, comprehend, and test. 
...
A second goal is to make the implementation of new automated refactorings as simple as possible, to enable interested developers to create their own refactorings. %To make the .. einstieg .. as simple as possible, the existing Scala compiler infrastructure is reused where possible.
...
\begin{description}
  ...
  \item[RenameAction] finally implements the \src{RefactoringAction} and contains all refactoring specific code. It instantiates the corresponding refactoring from the library and provides it to the generic \src{RefactoringAction}. The other refactorings are implemented analog by their respective actions.
