...
\paragraph{Inbound:} From the current enclosing scope's symbol, find all child symbols. From these, find all that are use inside the selected expressions and substract those that are defined in the selected expressions. This gives us all the inbound parameters. XXX
\paragraph{Outbound:} For each defined symbol inside the selection, check if it is used outside the selection.
...
This is the preferred method for IDEs that operate with change objects. Alternatively, if one just wants to generate the source code from a tree, the \src{createFragment} method can also be invoked directly. 
