...
This document describes the development of a library and associated tools to support automated refactoring of Scala programs. This master's thesis is a continuation of a prior term project \cite{ScalaRefactoring} at the University of Applied Sciences Rapperswil, Switzerland.
...
The breakthrough in the industry followed in 1999, when Martin Fowler and his colleagues published their very popular book \textit{Refactoring: Improving the Design of Existing Code} \cite{FowlerRefactoring}, where refactoring is defined as 

\begin{quote}
the process of changing a software system in such a way that it does not alter the external behavior of the code yet improves its internal structure.
\end{quote} 

Today, refactoring has been absorbed by the programming mainstream, and is usually well integrated into the developer's work-flow and development environments. Developers use refactoring tools to keep their code maintainable by applying refactorings such as Rename to quickly change identifiers. In agile environments, where the program is rapidly adapted to handle new requirements, performing regular refactorings is essential to achieve reusable code.

Refactoring as a technique does not mandate a tool or depend on a specific programming language.

\section{Scala}

The Scala programming language \cite{ProgrammingScala}, developed by Martin Odersky and his team at EPFL, is a statically typed language that runs on the Java virtual machine (or on .NET alternatively \cite{ScalacNet}) and excels with its unique combination of object-oriented and functional programming concepts. It has also been called a postfunctional language \cite{ScalaPostFunctional} because it has been designed ``to make functional constructs, imperative constructs, and objects all play well together''. 
...
The goal of this thesis is to support Scala IDEs with automated refactoring support. It aims to provide a comprehensive catalogue of refactorings and the necessary infrastructure to create new refactorings. To let (?) as many IDEs and other tools as possible profit from the project, it will provide an IDE independent refactoring library that only depends on the Scala compiler. IDEs can then integrated this library by providing the user interface and interaction. 

As most IDEs today are written in Java, integrating a Scala library is no problem. Also, because the majority of Scala IDEs are open source (NetBeans, Eclipse), having a single refactoring library allows the developers to cooperate on an implementation, not fragmenting the already scarce resources any further. As a showcase, this project provides the integration into the Eclipse based Scala IDE \cite{EclipseScalaIDE}.

Writing an automated refactoring is no easy task, several things have to be considered: one has to analyze the source code, create an appropriate representation (e.g. abstract or concrete syntax tree) of the program, transform it and turn it back into plain source code. 

The heart of a refactoring is the transformation or manipulation of the program representation; but often (from our experience with refactoring tools for languages like Ruby \cite{RubyRefactoring}, C++ \cite{CdtOopsla}, and Groovy \cite{GroovyOopsla}) the developer also has to provide the instructions on how these manipulations are to be translated back into source code. This makes creating new refactorings needlessly complex and is a high barrier to contributors. The Scala refactoring library tries to make this as simple as possible: code generation from the abstract syntax tree is implemented generically and with no input from the refactoring writer.

Transformations of the program are based on the Scala compiler's AST, and are written in a functional programming style that makes it possible to build complex transformations from simple ones.

To summarize, this project provides an IDE independent refactoring library which makes creating new refactorings as simple as possible. 
