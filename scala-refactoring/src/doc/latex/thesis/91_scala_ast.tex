...
\figref{figure:trees-class-diagram} shows a inheritance diagram of the various tree classes.
...
\noindent An object that can stand in for most other trees, it has no position, no type and no symbol. For \src{ValDefs}, the equivalent is the \src{emptyValDef} object.
...
The Scala language specification says ... and we can also see this when we print the \src{LabelDef}:
...
\noindent \src{Assign} trees are not used whenever a \src{=} is involved, only for non-initial assignments to variables. Calls to setter method are not represented by \src{Assign} trees.
...
\noindent The \src{Try} tree represents \src{try \ldots catch} expressions. Both the catchers and the finalizer are optional.
...
\noindent The \src{Typed} tree is used whenever a expression is annotated with a type. For example, in the following listing, the second and third occurences of \src{Int} are \src{Typed} trees:
...
\noindent \src{TypeApply} trees are used whenever a type is applied to a generic function. For example, in the following listing, both expressions on the left are represented by the same AST on the right.
...
\noindent \src{For} or sequence comprehensions are a purely syntatic construct; in the AST, they are represented with \src{foreach}, \src{withFilter}, \src{map}, and \src{flatMap} calls. The following listing shows some examples. 
