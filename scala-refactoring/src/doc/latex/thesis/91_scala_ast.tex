...
\paragraph{TypeTree} \member{AbsTypeTree <: TypTree}{\textit{original}: Tree}
...
The original type tree is still accessible via the \src{TypeTree}'s \src{original} member. Note that the standard tree \src{Traverser} and \src{Transformer} visitors do not handle this original type.
