...
class OrganizeImportsTest extends OrganizeImportsBaseTest {
  ...
  private def organizeForEnsime(pro: FileSet) = new OrganizeImportsRefatoring(pro) {
	  val oiConfig = OrganizeImports.OrganizeImportsConfig(
  ...
  def shouldPassForEnsime_v1() = {
    new FileSet {
      ...
      """
    } applyRefactoring organizeForEnsime
  }

  @Test
  def shouldPassForEnsime_v2() = {
    new FileSet {
      """
      import scala._
